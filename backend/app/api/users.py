...
from app.api.availability import validate_timezone
...
def update_profile(
  ...
  try:
    validate_timezone(fields.get("timezone"))
...
def get_availability(user: CurrentUser, conn: psycopg.Connection = Depends(get_db)):
  with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
    rows = cur.execute(
...
def set_availability(
  ...
  keys = [(slot.day, slot.hour) for slot in slots]
  if len(keys) != len(set(keys)):
    raise HTTPException(status_code=422, detail="Availability slots must be unique")
